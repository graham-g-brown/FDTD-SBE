...
if strcmp(system, 'SBE')
    ...
    [ H_k , P_k ] = SBE_bloch_basis_matrix_elements ( ) ;
...
function J_single = SBE_current_from_field ( E , t , H , P , k )
    ...
    for tdx = 2:N_t
        ...
        if tdx < N_t
            ...
            A2 = A ( t_n ) + 0.5 * dt * A ( t_n + 1 );
            A3 = F_of_t(t_n + 0.5*dt);
            A4 = F_of_t(t_n + dt);
        ...
        k1 = EOM_Velocity(rho                 , H , P , A1, dk, N_b , T2 ) ;
        k2 = EOM_Velocity(rho + 0.5 * dt * k1 , H , P , A2, dk, N_b , T2 ) ;
        k3 = EOM_Velocity(rho + 0.5 * dt * k2 , H , P , A3, dk, N_b , T2 ) ;
        k4 = EOM_Velocity(rho +       dt * k3 , H , P , A4, dk, N_b , T2 ) ;
        ...
        for m = 1:N_b_new
            for n = 1:N_b_new
                j_temp = j_temp + sum( rho_next(:,m,n) .* squeeze(P_k_new(:,n,m)) );
            end
        end
...
function [ H_k , P_k ] = SBE_bloch_basis_matrix_elements ( ) 
    ...
    % Number of bands
    N_b  = 2; % the more bands, the heaviear the computation is later on in the HHG calculations.
    ...
    D1_x = DFT_inv * (-1i*diag(k_x)) * DFT;
    
    % Allocate matrices
    P_k = zeros(N_k, N_b, N_b);
    H_k = zeros(N_k, N_b, N_b);
    
    for kdx = 1:N_k
        for bdx = 1:N_b
            for bbdx = 1:N_b
                P_k(kdx,bdx,bbdx) = sum( -1i * conj( squeeze(umk(kdx, bbdx, :)) ) ...
                          .* (D1_x * squeeze(umk(kdx, bdx, :))) ) * dx;
                if bdx == bbdx
